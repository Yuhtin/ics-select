...
for (const theme of ['light', 'dark'] as const) {
  test(`member outcome form has mobile touch targets and sans validation in ${theme}`, async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot(`academy-item-choice-${theme}-mobile.png`, { animations: 'disabled' });
    await page.getByRole('button', { name: /Nailed it$/ }).click();
    await page.getByRole('button', { name: 'Continue', exact: true }).click();
    await page.getByRole('textbox', { name: /nota/i }).fill('Entendi o invariante. Vou revisar os limites antes da próxima tentativa.');
    await page.getByRole('heading', { name: 'Sua nota' }).scrollIntoViewIfNeeded();
    await expect(page).toHaveScreenshot(`academy-item-reflection-${theme}-mobile.png`, { animations: 'disabled' });
    await page.getByRole('button', { name: 'Continue', exact: true }).click();
