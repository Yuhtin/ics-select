...
export const websocketRpcBlockchain: Lesson = {
  ...
  nodes: [
    ...
    },
    {
      ...
      pass1:
        '**RPC** quer dizer Remote Procedure Call, ou chamada de procedimento remoto: chamar uma função que roda em outra máquina como se ela fosse local. Você manda o nome do método e a lista de argumentos, a outra máquina executa e devolve o retorno. O **JSON-RPC** é uma das formas de escrever esse pedido (gRPC e XML-RPC são outras), e é a que o Ethereum adotou.\n\nTodo pedido para um node tem a mesma forma, definida pelo padrão JSON-RPC 2.0: um objeto com `jsonrpc`, `method`, `params` e `id`. O node responde com um objeto que carrega o mesmo `id`, e é assim que você casa uma resposta com a pergunta que a originou. É simples de propósito, porque a graça está em outro lugar: esse mesmo envelope viaja por HTTP ou por WebSocket sem mudar nada.',
      pass2:
    ...
    // ──────────────── A CONEXÃO VIVA ────────────────
    {
      ...
      pass1:
        '**WebSocket** é um protocolo, definido na RFC 6455, que estabelece um canal de comunicação **bidirecional** e **persistente** entre dois pontos, sobre uma única conexão TCP. Bidirecional porque os dois lados mandam mensagem quando quiserem, sem precisar de convite. Persistente porque abre uma vez e continua aberto, em vez de uma conexão nova por mensagem.\n\nEle resolve exatamente o problema do beat 1: ele deixa o servidor falar primeiro. E o truque é elegante. Ele **começa** como uma requisição HTTP comum, com um cabeçalho pedindo `Upgrade: websocket`. O servidor responde `101 Switching Protocols`, e aquele mesmo socket TCP para de falar HTTP e passa a trocar frames nos dois sentidos. É por isso que o WebSocket atravessa firewall e proxy corporativo: na porta de entrada, ele parecia HTTP.',
      pass2:
