...
export const vocabularioIa: Lesson = {
  ...
  nodes: [
    ...
    },
    {
      ...
      oneLine: 'A IA não sabe o que você sabe, ela só tem o que está no contexto, e o contexto é finito.',
      pass1: 'Tudo que o modelo enxerga de uma vez cabe numa contexto: as instruções do sistema, o histórico da conversa, os arquivos que entraram, o resultado de ferramentas, e o seu texto. Esse contexto tem tamanho, e esse tamanho tem nome: janela de contexto. Quando enche, o começo some ou é comprimido. Sessão é a conversa inteira; turno é cada rodada dentro dela. E como o contexto é finito, o que você põe nela tem anatomia: fecha com PTCF, persona, task, context, format.',
      pass2: '**A frase que resume tudo**\nA IA não sabe o que você sabe, ela só tem o que está no contexto. Tudo que o modelo enxerga num turno cabe nesse contexto: as instruções do sistema, o histórico da conversa até ali, os arquivos que entraram, o resultado do que alguma ferramenta trouxe, e o seu texto agora. Nada além disso existe pra ele naquele momento.\n\n**O contexto tem tamanho, e o tamanho tem nome**\nAquele limite do beat anterior agora ganha nome: janela de contexto. É o tamanho máximo do contexto, medido em token. Quando a conversa cresce mais do que esse tamanho, alguma coisa precisa sair: o começo da conversa some ou é comprimido pra abrir espaço pro que é novo.\n\n**Sessão e turno**\nSessão é a conversa inteira, do primeiro ao último turno. Turno é cada rodada dentro dela: você escreve, ele responde, isso é um turno. Uma sessão longa acumula turno atrás de turno, e cada um deles fica em cima do contexto até ele encher.\n\n**Contexto irrelevante não é neutro**\nColar informação que não serve pra nada não é grátis: ela ocupa espaço no contexto e ainda piora a resposta, porque o modelo tem que separar o que importa em meio a mais coisa. Uma contexto cheio de coisa inútil deixa menos espaço pro que realmente importa.\n\n**PTCF: a anatomia do que você põe no contexto**\nJá que o contexto é finito, o que você põe nela tem estrutura, não é só um monte de texto solto. PTCF quer dizer Persona, quem o modelo deve agir como, Task, a tarefa exata, Context, o material que ele precisa pra fazer essa tarefa, e Format, como a resposta deve sair. Um pedido fraco, tipo `me ajuda com esse código`, não define nenhum dos quatro. Um pedido forte nomeia os quatro: `aja como revisor de código sênior do Academy Fellow` (persona), `revise a função de cálculo de saldo abaixo` (task), `aqui está o arquivo e o teste que está falhando` (context), `responda em bullets, no máximo cinco linhas por item` (format).',
      visuals: [
        {
          kind: 'ascii',
          title: 'O contexto enchendo ao longo da conversa',
          art: 'TURNO 1 (contexto quase vazio)\n+------------------------------+\n| instrucoes do sistema        |\n| seu texto                    |\n+------------------------------+\n\nTURNO 4 (contexto enchendo)\n+------------------------------+\n| instrucoes do sistema        |\n| historico (turnos 1 a 3)     |\n| arquivo.ts                   |\n| seu texto                    |\n+------------------------------+\n\nTURNO 9 (contexto quase cheio)\n+------------------------------+\n| instrucoes do sistema        |\n| historico (turnos 1 a 8)     |\n| arquivo.ts                   |\n| resultado de ferramenta      |\n| seu texto                    |\n+------------------------------+',
          caption: 'A cada turno novo, o histórico anterior continua ocupando espaço no contexto; por isso ele enche mesmo sem ninguém colar nada novo de propósito.',
        ...
        },
        {
          ...
          caption: 'Pedido fraco: `me ajuda com esse código`. Pedido forte com PTCF: Persona (azul) `aja como revisor de código sênior do Academy Fellow`; Task (rosa) `revise a função de cálculo de saldo abaixo`; Context (amarelo) `aqui está o arquivo e o teste que está falhando`; Format (verde) `responda em bullets, no máximo cinco linhas por item`.',
      ...
      pass3: [
        {
          ...
          note: 'É só o tamanho do contexto naquele momento; quando a sessão acaba ou o contexto enche e o começo sai, isso não fica guardado em lugar nenhum.',
        },
        {
        ...
        },
        {
          ...
          note: 'Sessão é a conversa inteira; turno é cada rodada dentro dela, e uma sessão longa tem muitos turnos empilhados na mesmo contexto.',
        },
      ],
      anchor: 'Se o contexto já tem instruções do sistema, histórico, arquivos e resultado de ferramentas empilhados, o que sobra dela pra caber a sua pergunta de agora quando a sessão já está longa?',
      followup: 'Se o contexto é finito e o modelo só prevê o próximo token olhando pro que está nela, o que precisa acontecer pra ele conseguir ler um arquivo que ainda não está no contexto?',
    ...
    // ──────────────── FERRAMENTAS · agente, skill, MCP, harness, peças ────────────────
    {
      ...
      oneLine: 'O modelo nunca executa nada, só pede: o agente é o loop que pega esse pedido, manda executar de verdade e devolve o resultado pro contexto.',
      pass1: 'O beat anterior perguntou como um arquivo que ainda não está no contexto chega lá. A resposta é o agente: não um modelo diferente, mas o mesmo previsor de token do beat 2 rodando dentro de um loop. A cada volta ele lê a contexto inteiro e devolve texto, e parte desse texto pode ser um pedido de ferramenta. Algo fora do modelo executa esse pedido de verdade e cola o resultado de volta no contexto. O modelo nunca toca o disco, ele só pede.',
      pass2: '**O mesmo previsor, agora em loop**\n\nO agente não é um modelo diferente do beat 2. É o mesmo previsor de token, sem memória, rodando dentro de um programa que repete uma pergunta pra ele várias vezes seguidas: dado o que está no contexto agora, qual é o próximo pedaço de texto. A diferença toda está em volta do modelo, não dentro dele.\n\n**O pedido de ferramenta**\n\nParte do texto que o modelo devolve não é resposta pro humano, é um pedido estruturado, algo como ler tal arquivo ou rodar tal busca. O modelo produz esse pedido do mesmo jeito que produz qualquer outro token, previsto um atrás do outro. Ele não sabe se aquele pedido vai funcionar, só sabe que aquele é o próximo pedaço de texto mais provável.\n\n**Quem executa de verdade**\n\nAlguém fora do modelo, o código que roda o loop, pega esse pedido e executa contra o mundo real: abre o arquivo, chama a busca, roda o comando. O resultado bruto volta e vira parte do contexto, como se o modelo sempre tivesse sabido aquilo. Na próxima volta ele lê o contexto de novo, agora maior.\n\n**Quem decide parar**\n\nNão existe um número fixo de voltas. A cada turno o próprio modelo decide, olhando pro contexto como ele está naquele momento, se já tem o suficiente pra responder ou se precisa pedir mais uma ferramenta. Essa decisão é a parte difícil: parar cedo demais deixa a tarefa pela metade, e não parar vira loop sem fim gastando token atrás de token.\n\n**Sugerir não é fazer**\n\nUm chat que sugere apagar um arquivo não apaga nada, o texto só fica na tela. Um agente que chama a ferramenta de apagar, apaga de verdade. É exatamente nesse ponto, o pedido de ferramenta virando execução real, que o risco aparece: o que um agente pode sugerir é livre, o que ele pode de fato fazer depende de quais ferramentas estão na mão dele.',
      visuals: [
        {
          kind: 'ascii',
          title: 'O loop do agente',
          art: '        +-------------+\n   +--->|  CONTEXTO   |\n   |    | tudo que ele|\n   |    |   ve agora  |\n   |    +-------------+\n   |           |\n   |           v\n   |    +-------------+\n   |    |   MODELO    |\n   |    | preve texto |\n   |    +-------------+\n   |           |\n   |    pedido de ferramenta\n   |           v\n   |    +-------------+\n   |    |  FERRAMENTA |\n   |    | executa de  |\n   |    |   verdade   |\n   |    +-------------+\n   |           |\n   |    resultado volta\n   |           v\n   +----- pro contexto, loop de novo',
          caption: 'O mesmo previsor do beat 2, agora dentro de um loop que executa de verdade e devolve o resultado pro contexto.',
      ...
      pass3: [
        ...
        },
        {
          ...
          note: 'Nunca. Ele pede a leitura, algo fora do modelo executa de verdade e cola o resultado no contexto. Se essa ferramenta falhar ou não existir, o modelo só vê o erro que voltou, não o arquivo.',
        },
        {
          gotcha: 'Existe um número fixo de voltas que o loop roda antes de responder.',
          note: 'Não existe. O próprio modelo decide a cada turno se já tem o suficiente no contexto pra responder ou se pede mais uma ferramenta, e essa decisão pode falhar pros dois lados: parar cedo demais ou repetir chamada sem necessidade.',
        },
        {
      ...
      anchor: 'Quando alguém descreve a IA lendo um arquivo sozinha no meio de uma tarefa longa, o que exatamente aconteceu entre o pedido dela e o conteúdo aparecer no contexto?',
      followup: 'Se o agente decide sozinho, turno a turno, quando parar de pedir ferramenta, o que decide QUAIS instruções já chegam prontas no contexto antes mesmo da primeira pergunta do usuário?',
    ...
    // prompt-master com 13 templates e 37 padrões, grill-me com 6 linhas e grilling com 12.
    {
      ...
      pass3: [
        ...
        {
          gotcha: 'Ter mais skills instaladas deixa o contexto mais cheio o tempo todo.',
          note: 'Só o description de cada skill fica sempre disponível pro modelo olhar. O corpo inteiro da instrução só entra no contexto quando aquele description bate com o pedido, então ter 37 skills instaladas não enche o contexto sozinho.',
        },
        {
      ...
      pass3: [
        {
          ...
          note: 'Não sai. O harness muda o system prompt, o catálogo de ferramentas e a regra de contexto cheio, e isso muda a resposta mesmo com o modelo idêntico rodando por baixo dos dois.',
        },
        {
        ...
        },
        {
          ...
          note: 'Importa e muito: o harness decide o que entra no contexto, mas quem prevê o próximo token, pode alucinar, ter janela menor ou custar mais por token continua sendo o modelo, como nos beats 1 e 2.',
        },
      ],
    ...
    // as duas ressalvas literais na seção "How Savings Work", superpowers com 14 skills.
    {
      ...
      pass3: [
        ...
        },
        {
          ...
          note: 'Não. Bash output é só uma fatia do que entra no contexto, ao lado do prompt, do system prompt e do histórico. O próprio README avisa disso.',
        },
        {
        ...
        },
        {
          ...
          note: 'Tem custo: ele começa com a contexto vazio e não viu nada da sua sessão principal. Quem manda a tarefa precisa escrever pra ele tudo que importa.',
        },
      ],
    ...
    // GLM-5.2 (753B), ambos MIT. O GLM-5.3 completo saiu por API antes dos pesos.
    {
      ...
      followup: 'Se hoje já dá pra guardar peso, modelo e ferramenta local na sua própria máquina, o que acontece quando o que você quer guardar não é um modelo, e sim um projeto inteiro, com milhões de palavras espalhadas em milhares de arquivo, que não cabe em nenhum contexto?',
    ...
    },
    {
      ...
      oneLine: 'O contexto de um projeto inteiro vira grafo de entidade e relação, salvo entre sessões, pra IA consultar sem te obrigar a caber tudo numa contexto só.',
      ...
      pass3: [
        ...
        {
          gotcha: 'Um grafo grande resolve o limite de leitura só juntando tudo numa contexto maior.',
      ...
      anchor: 'Se um projeto tem quase 3 milhões de palavras espalhadas em 2433 arquivos e nenhum contexto cabe isso, o que exatamente você constrói pra não perder a ligação entre as partes?',
    ...
    // ──────────────── FECHAMENTO · turno, fluxo, setup ────────────────
    {
      ...
      pass1: 'Um turno é o ciclo inteiro entre você apertar enter e a resposta parar de escrever na tela, e cada caixa desse ciclo já tem nome nesta escada. Você digita, o harness monta o contexto com o system prompt dele, o seu CLAUDE.md, as skills que o assunto ativou e o histórico da conversa, o modelo devolve token atrás de token até pedir uma ferramenta, o harness executa de verdade e o resultado volta pro contexto, e o ciclo recomeça. Quando o contexto enche, alguma coisa precisa dar lugar.',
      pass2: '**O contexto se monta antes do modelo ver qualquer coisa**\n\nAntes do modelo processar uma única palavra sua, o harness já montou a contexto inteiro. Primeiro entra o system prompt dele, fixo pra toda sessão. Depois entra o seu CLAUDE.md, se o projeto tiver um. Depois entram as skills que o assunto da sua mensagem ativou, cada uma como um arquivo a mais na pilha. Depois entra o histórico da conversa, tudo que já foi dito nesse turno pra trás. Só por último entra o seu texto, a última coisa a chegar no contexto antes do modelo começar a prever o próximo token.\n\n**O modelo só sabe prever o próximo token**\n\nO modelo não decide nada fora disso. Ele lê a contexto inteiro e devolve token atrás de token, e de vez em quando esses tokens formam um pedido de ferramenta, um tool call. É a mesma peça do beat 4: o modelo pede, ele não executa. Quem decide se aquele pedido vira ação de verdade é o harness.\n\n**O harness executa, e um hook pode interceptar bem ali**\n\nÉ aqui que o pedido vira execução de verdade: leitura de arquivo, comando de terminal, chamada num sistema externo. É exatamente nesse ponto, entre o pedido e a execução, que um hook pode interceptar, agindo sobre o que roda de fato, do jeito que o hook do rtk reescreve um comando de shell antes dele sair do lugar.\n\n**O resultado volta pro contexto e o ciclo recomeça**\n\nO resultado da ferramenta volta pro contexto como mais uma linha, e o modelo lê o contexto de novo, agora com essa linha a mais, e decide se pede outra ferramenta ou já responde. É o loop do beat 4 rodando turno adentro, passo 3 ao passo 6, quantas vezes o critério de parada permitir.\n\n**Quando o contexto enche**\n\nSe o turno se arrasta com muita ferramenta chamada em sequência, o contexto enche antes da tarefa acabar. Sobram duas saídas: comprimir o que já está ali, resumindo o histórico pra abrir espaço, ou jogar a parte suja do trabalho pra um subagente, que abre a própria contexto vazio, faz o trabalho sujo lá e devolve só o resultado limpo pro turno principal. As duas saídas existem pelo mesmo motivo: o contexto é finito, e finito quer dizer que ele acaba enchendo.',
      diagramUrl: '/diagrams/vocabulario-ia/beat-11-turno.png',
      pass3: [
        {
          gotcha: 'Pensar que o hook intercepta o modelo.',
          note: 'O hook fica entre o pedido de ferramenta e a execução real feita pelo harness. O modelo nunca sabe que o hook existe, ele só vê o resultado que volta pro contexto.',
        },
        {
        ...
        {
          gotcha: 'Achar que o modelo decide comprimir o contexto ou chamar um subagente.',
          note: 'Essa decisão é do harness, seguindo a lógica programada pra quando o contexto enche. O modelo só continua prevendo token com o que estiver no contexto naquele momento.',
        },
        {
    ...
    // ──────────────── SÍNTESE ────────────────
    {
      ...
      pass2: '**Modelo e harness não são a mesma coisa**\n\nO modelo é o previsor de token, um monte de pesos fixos que não guarda nada de uma sessão pra outra. O harness é o programa que roda o loop em volta dele: monta o contexto, executa o pedido de ferramenta, decide quando comprimir. Hermes prova isso melhor que qualquer slide, porque é nome de harness e nome de família de modelo ao mesmo tempo, os dois da Nous Research. Saber separar qual é qual, na hora, é o teste de verdade de que você entendeu a diferença.\n\n**Contexto faltando ou contexto sobrando**\n\nQuase todo problema prático com IA cai numa dessas duas caixas. Contexto faltando é o modelo respondendo sem saber o que você sabe, e skill e MCP existem pra resolver isso, um trazendo instrução sob demanda, o outro trazendo dado e sistema real. Contexto sobrando é o contexto enchendo até parar de caber o que importa, e hook, plugin, subagente e grafo existem pra resolver isso de formas diferentes: filtrando o que entra, empacotando o que se repete, isolando o trabalho sujo numa contexto própria, ou guardando o projeto inteiro fora do contexto, num grafo que a IA consulta sob demanda.\n\n**A pergunta que sobra depois da aula**\n\nToda peça nova de IA que aparecer depois de hoje carrega as duas mesmas perguntas. Isso é modelo ou é harness? Isso resolve contexto vazio ou contexto cheio? Quem sabe responder as duas já sabe onde encaixar a peça nova sem precisar decorar o nome dela primeiro.',
      pass3: [
        {
        ...
        {
          gotcha: 'Achar que resolver contexto cheio é sempre comprimir.',
          note: 'Às vezes é subagente, às vezes é grafo, às vezes é simplesmente não colocar aquilo no contexto desde o início.',
        },
        {
          gotcha: 'Achar que peso aberto resolve o problema de contexto sozinho.',
          note: 'Peso aberto muda onde o modelo roda. Contexto cheio ou vazio é um eixo separado, resolvido por peças diferentes.',
        },
      ],
      anchor: 'Se eu perguntar qual peça dessa escada resolve contexto vazio e qual resolve contexto cheio, você consegue separar as seis numa lista rápida agora?',
      followup: 'Da próxima vez que uma ferramenta de IA prometer alguma coisa nova, qual das duas perguntas dessa aula você vai fazer primeiro, isso é modelo ou harness, ou isso resolve contexto vazio ou contexto cheio?',
  ...
  glossary: [
    {
      ...
      terms: [
        ...
        { term: 'sessão', definition: 'A conversa inteira, do primeiro ao último turno. Sessão nova começa com a contexto limpo.' },
        ...
        { term: 'prompt', definition: 'O texto que você põe no contexto. É a única parte do contexto que você escreve diretamente.' },
    ...
    },
    {
      ...
      terms: [
        ...
        { term: 'subagente', definition: 'Um agente disparado com contexto isolado, para a parte que sujaria muito o contexto. O contexto dele começa limpo, e é por isso que ele precisa receber tudo que importa.' },
      ],
    },
    {
      title: 'Avançado',
