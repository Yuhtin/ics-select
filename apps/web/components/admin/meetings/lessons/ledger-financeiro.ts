...
armazenamento por idade do dado:
  ...
  entries antigos  ---> store barato   (TerraBlob)
      ...
      pass1:
        'Hora de juntar as quatro peças num desenho. O passageiro encerra a corrida e, a partir dali, o dinheiro atravessa uma sequência de camadas até virar um par de entries imutáveis e depois aparecer como saldo para o motorista. A decisão que organiza todo o resto é separar o **write path**, que grava o pagamento e precisa ser exato e durável, do **read path**, que mostra histórico e saldo e tolera atraso. Os dois têm requisitos opostos, então recebem soluções diferentes. O sistema que faz isso na Uber é o LedgerStore, com 5 bilhões de eventos por dia sobre mais de 1 trilhão de entries.',
      pass2:
        'O desenho completo está no primeiro visual abaixo. O que vale explicar em voz alta é por que os dois caminhos divergem.\n\n**Por que a escrita é cara.** A autorização de cartão precisa ler de volta o que acabou de gravar, o que se chama **read-your-write**. É o único requisito da aula que exige um índice **strongly consistent**, mantido com **two-phase commit**. Sem ele, uma segunda autorização pode não enxergar a primeira e aprovar uma cobrança que deveria ser negada.\n\n**Por que a leitura é barata.** O motorista ver um lançamento 200 ms depois não muda decisão nenhuma. Então os índices do read path são **eventually consistent**, mantidos como materialized view atualizada em background, e o saldo vem de snapshot mais delta.\n\n**Por que existe um terceiro índice.** A pergunta do auditor, "todas as transações de março", não é leitura de app nem escrita. Ela usa um índice particionado por timestamp. Sem ele, a consulta vira varredura do log inteiro.\n\n**A escala que isso sustenta.** A migração que consolidou o desenho moveu 1,2 petabyte comprimido e mais de 1 trilhão de entries, sem inconsistência detectada em seis meses de validação.\n\n**O que o desenho não faz.** Não elimina o custo de ler saldo, apenas o desloca para o snapshot. Não dispensa a idempotency key na borda, porque nenhuma camada de baixo distingue reenvio de pagamento novo. E não sobrevive a alguém escrever no store por fora do serviço, que é exatamente por que a soma zero precisa estar declarada no banco.',
      pass3: [
        {
...
o que essa escada cobrou:
  ...
  glossary: [
    ...
    },
    {
      ...
      terms: [
        { term: 'LedgerStore', definition: 'O ledger da Uber. Mais de 1 trilhão de entries e 5 bilhões de eventos por dia.' },
        { term: 'Docstore', definition: 'O armazenamento rápido da Uber, onde os entries recentes ficam.' },
        { term: 'TerraBlob', definition: 'O armazenamento barato da Uber, para onde os entries antigos descem.' },
