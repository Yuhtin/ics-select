...
export function CohortKnowledgeGrid({
  ...
  const cellColor = (c: Cell | undefined) => {
    if (!c || c.itemsDone === 0) return inverted ? 'text-white/30' : 'text-fg-faint';
    if (c.itemsDone === 1) return inverted ? 'text-white/70' : 'text-fg-soft';
    return inverted ? 'text-white' : 'text-fg';
  };
  ...
  if (members.length === 0) {
    return (
      <div className={`font-sans text-xs ${inverted ? 'text-white/60' : 'text-fg-mute'}`}>
  ...
  return (
    <div className="overflow-x-auto">
      <table className="font-sans text-xs" style={{ borderCollapse: 'separate' }}>
        <thead>
          <tr style={{ height: 120 }}>
              ...
              >
                <div
                  className={`pointer-events-none absolute left-1/2 bottom-2 origin-bottom-left whitespace-nowrap text-[10px] uppercase tracking-label ${
                    inverted ? 'text-white/70' : 'text-fg-soft'
                  }`}
        ...
        <tbody>
          {members.map((m) => (
            <tr
              key={m.userId}
              className={inverted ? 'hover:bg-white/5' : 'hover:bg-bg-subtle'}
            >
              <td
          ...
          {showTotals && (
            <tr
              className={inverted ? 'border-t border-white/20' : 'border-t border-border-token'}
            >
              <td
                className={`pt-2 pr-3 uppercase tracking-label text-[10px] ${
                  inverted ? 'text-white/60' : 'text-fg-mute'
                }`}
              >
                Total
              </td>
              {totals.map((n, i) => (
                <td
                  key={i}
                  className={`pt-2 px-2 text-center font-mono tabular-nums ${
                    inverted ? 'text-white/80' : 'text-fg-soft'
                  } ${
                    i > 0 && i % 4 === 0
                      ? inverted
                        ? 'border-l border-white/20'
