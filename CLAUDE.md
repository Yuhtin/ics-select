...
**Curation workflow.** The project-local skill `.claude/skills/ics-library-curate/SKILL.md` encodes the layered EASY/MEDIUM/HARD ladder, approved-channels whitelist, kind-tag vocabulary (`concept`/`tradeoffs`/`practice`/`case-study`/`career`), exact-YouTube-duration rule (scrape `lengthSeconds` via `curl`), and book whitelist (Grokking Data Structures / Algorithms / Deep Learning only). The `career` kind was added 2026-08-05 for role-guide content ("what a cloud engineer actually does"); it lives in `foundations` with the domain topic as a cover, and does not reopen the door to soft-skills/motivational video. When adding a new item manually, set `topicSlugs: [primary, ...covers]` in the seed — the first slug is the primary, the rest are covers.
...
| Retros submitted | 21 | `min(1, retros / max(0, weeksElapsed - 1)) × 21`, with full credit (21pts) when `weeksElapsed ≤ 1`. The divisor is `weeksElapsed - 1` because the retro for week N opens Friday of week N — until that week ENDS, the current week's retro window is either not open or still in progress, so it doesn't count as a missed opportunity. Example: in week 2 with 1 retro submitted, score is `min(1, 1 / (2-1)) × 21 = 21pts` (max), not `1/2 × 21 = 10.5` as the older formula gave. Same divisor used by `behavior.retros.expected` in the cockpit service so the "↓ N missing" hint matches the score. |
